...
class PlutoProvider(BaseProvider):
    def __init__(self):
        ...
        self.x_forward = {
            "local": "", "uk": "178.238.11.6", "ca": "192.206.151.131", 
            "fr": "193.169.64.141", "us_east": "108.82.206.181", "us_west": "76.81.9.69",
        }
        
        self.headers = {
            'authority': 'boot.pluto.tv', 'accept': '*/*', 'accept-language': 'en-US,en;q=0.9',
            'origin': 'https://pluto.tv', 'referer': 'https://pluto.tv/', 'user-agent': self.get_user_agent(),
        }
            ...
            for channel in channel_data:
                ...
                for image in channel.get('images', []):
                    if image.get('type') == 'colorLogoPNG':
                        logo = image.get('url', ''); break
                group = categories_list.get(channel_id, 'General')
                quality_suffix = ("&quality=720p&deviceMake=chrome&deviceType=web&deviceModel=web"
        ...
        with open(file, "r", encoding="utf-8") as f:
            for line in f:
                if line.startswith("#EXTINF"):
                    line = line.replace('group-title="', f'group-title="[{display_label}] ').replace(',', f',[{display_label}] ')
                    master_content += line
